...
class TeanoGraph_pro:
    def __init__(self, u_grade):
        ...
        self.universal_grid_matrix_T = theano.shared(np.array([[0., 0.],
                                                               [0., 0.],
                                                               [0., 0.204082],
                                                               [0., 0.],
                                                               [0., 0.],
        ...
        self.yet_simulated = T.ones(self.grid_val_T.shape[0])

        # Init values
        self.n_dimensions = 3


       # self.grade_universal = self.u_grade_T

        # Extra parameters
        self.i_reescale = 1 / (self.c_resc ** 2)
        self.gi_reescale = 1 / self.c_resc
    ...
    def matrices_shapes(self):
        ...
        length_of_C = self.length_of_CG + self.length_of_CGI + self.length_of_U_I
    ...
    def cov_interfaces(self):
        ...
        SED_rest_rest = (T.sqrt(
            ...
            2 * self.rest_layer_points.dot(self.rest_layer_points.T))).astype("float32")

        SED_ref_rest = (T.sqrt(
            (self.ref_layer_points ** 2).sum(1).reshape((self.ref_layer_points.shape[0], 1)) +
            (self.rest_layer_points ** 2).sum(1).reshape((1, self.rest_layer_points.shape[0])) -
            2 * self.ref_layer_points.dot(self.rest_layer_points.T))).astype("float32")

        SED_rest_ref = (T.sqrt(
            (self.rest_layer_points ** 2).sum(1).reshape((self.rest_layer_points.shape[0], 1)) +
            (self.ref_layer_points ** 2).sum(1).reshape((1, self.ref_layer_points.shape[0])) -
            2 * self.rest_layer_points.dot(self.ref_layer_points.T))).astype("float32")

        SED_ref_ref = (T.sqrt(
            (self.ref_layer_points ** 2).sum(1).reshape((self.ref_layer_points.shape[0], 1)) +
            (self.ref_layer_points ** 2).sum(1).reshape((1, self.ref_layer_points.shape[0])) -
            2 * self.ref_layer_points.dot(self.ref_layer_points.T))).astype("float32")
    ...
    def cov_gradients(self, verbose=0):
        ...
        SED_dips_dips = (T.sqrt(
            (self.dips_position ** 2).sum(1).reshape((self.dips_position.shape[0], 1)) +
            (self.dips_position ** 2).sum(1).reshape((1, self.dips_position.shape[0])) -
            2 * self.dips_position.dot(self.dips_position.T))).astype("float32")
    ...
    def cov_interface_gradients(self, verbose=0):
        ...
        SED_dips_rest = (T.sqrt(
            (self.dips_position ** 2).sum(1).reshape((self.dips_position.shape[0], 1)) +
            (self.rest_layer_points ** 2).sum(1).reshape((1, self.rest_layer_points.shape[0])) -
            2 * self.dips_position.dot(self.rest_layer_points.T))).astype("float32")

        SED_dips_ref = (T.sqrt(
            (self.dips_position ** 2).sum(1).reshape((self.dips_position.shape[0], 1)) +
            (self.ref_layer_points ** 2).sum(1).reshape((1, self.ref_layer_points.shape[0])) -
            2 * self.dips_position.dot(self.ref_layer_points.T))).astype("float32")
    ...
    def x_to_interpolate(self, verbose=0):
        ...
        grid_val.name = '3D coordinates where we are going to interpolate'
        if verbose > 1:
            theano.printing.pydotprint(C_G, outfile="graphs/" + sys._getframe().f_code.co_name + ".png",
                                       var_with_name_simple=True)
    ...
    def gradient_contribution(self):
        ...
        SED_dips_SimPoint = (T.sqrt(
            ...
            2 * self.dips_position.dot(grid_val.T))).astype("float32")
    ...
    def interface_contribution(self):
        ...
        SED_rest_SimPoint = (T.sqrt(
            (self.rest_layer_points ** 2).sum(1).reshape((self.rest_layer_points.shape[0], 1)) +
            (grid_val ** 2).sum(1).reshape((1, grid_val.shape[0])) -
            2 * self.rest_layer_points.dot(grid_val.T))).astype("float32")

        SED_ref_SimPoint = (T.sqrt(
            (self.ref_layer_points ** 2).sum(1).reshape((self.ref_layer_points.shape[0], 1)) +
            (grid_val ** 2).sum(1).reshape((1, grid_val.shape[0])) -
            2 * self.ref_layer_points.dot(grid_val.T))).astype("float32")
    ...
    def potential_field_at_interfaces(self):
        ...
        potential_field_interfaces_unique, updates1 = theano.scan(
            ...
            outputs_info=None,
            sequences=dict(input=T.concatenate((T.stack(0), self.number_of_points_per_formation_T)),
                           taps=[0, 1]),
