...
u_grade_T = T.scalar("grade of the universal drift")#= theano.shared(9, "grade of the universal drift", allow_downcast=True)


rescaling_factor_T = theano.shared(1, "Rescaling factor", allow_downcast=True)
grid_val_T = theano.shared(np.array([[0, 0., 0.],
                                    [0, 0., 0.204082]]), allow_downcast=True)

final_block = theano.shared(np.zeros_like(np.array([0, 0., 0.])),
                            "Final block computed", allow_downcast=True)

nugget_effect_grad_T = theano.shared(0.01, allow_downcast=True)
number_of_points_per_formation_T = theano.shared(np.zeros(2), allow_downcast=True)
a_T = theano.shared(1, allow_downcast=True)
c_o_T = theano.shared(1, allow_downcast=True)
universal_matrix_T = theano.shared(np.array([[0., 0.],
                                             [0., 0.],
                                             [0., 0.204082],
                                             [0., 0.],
                                             [0., 0.],
...
# Extra parameters
i_reescale = 1 / (rescaling_factor_T ** 2)
gi_reescale = 1 / rescaling_factor_T
...
grid_val = T.vertical_stack((grid_val_T * yet_simulated.reshape(
...
C_G = T.switch(
    ...
    (  # else, following Chiles book
        ...
        ((
             (SED_dips_dips < a_T) *  # first derivative
             (-c_o_T * ((-14 / a_T ** 2) + 105 / 4 * SED_dips_dips / a_T ** 3 -
                             35 / 2 * SED_dips_dips ** 3 / a_T ** 5 +
                             21 / 4 * SED_dips_dips ** 5 / a_T ** 7))) +
         (SED_dips_dips < a_T) *  # Second derivative
         c_o_T * 7 * (9 * SED_dips_dips ** 5 - 20 * a_T ** 2 * SED_dips_dips ** 3 +
                           15 * a_T ** 4 * SED_dips_dips - 4 * a_T ** 5) / (2 * a_T ** 7)) -
...
C_G = T.fill_diagonal(C_G, -c_o_T * (-14 / a_T ** 2) + nugget_effect_grad_T)

# Cross-Covariance gradients-interfaces
C_GI = gi_reescale * (
    (hu_rest *
     (SED_dips_rest < a_T) *  # first derivative
     (- c_o_T * ((-14 / a_T ** 2) + 105 / 4 * SED_dips_rest / a_T ** 3 -
                      35 / 2 * SED_dips_rest ** 3 / a_T ** 5 +
                      21 / 4 * SED_dips_rest ** 5 / a_T ** 7))) -
    (hu_ref *
     (SED_dips_ref < a_T) *  # first derivative
     (- c_o_T * ((-14 / a_T ** 2) + 105 / 4 * SED_dips_ref / a_T ** 3 -
...
sigma_0_grad = T.sum(
    (weights[:length_of_CG, :] *
     gi_reescale *
     (-hu_SimPoint *
      (SED_dips_SimPoint < a_T) *  # first derivative
      (- c_o_T * ((-14 / a_T ** 2) + 105 / 4 * SED_dips_SimPoint / a_T ** 3 -
...
universal_matrix = T.horizontal_stack(
    (universal_matrix_T * yet_simulated).nonzero_values().reshape((9, -1)),
    _universal_terms_layers)

if u_grade_T.get_value() == 0:
    f_0 = 0
else:
...
potential_field_unique, updates1 = theano.scan(fn=average_potential,
                                               ...
                                               sequences=dict(
                                                   input=T.concatenate(
                                                       (T.stack(0),
                                                        number_of_points_per_formation_T)),
                                                   taps=[0, 1]),
