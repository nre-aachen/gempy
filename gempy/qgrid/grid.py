...
def show_grid(data_frame, show_toolbar=None, remote_js=None, precision=None, grid_options=None, export_mode=None, a = 0):
    """
    ...
    if show_toolbar:
       ...
       #  export.on_click(grid.export)
        update_table = widgets.Button(description="Update Table")
        def exit(but, value=0):
            value += 1
            print(value)
            return value
        update_table.on_click(exit)

        display(widgets.HBox((add_row, rem_row, update_table)), grid)
    else:
        if export_mode:
            grid.export()
        else:
